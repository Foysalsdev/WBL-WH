...
// ═══════════════════════════════════════════════════════════════
//  InvoiceDialog — Step 3: generate invoice
// ═══════════════════════════════════════════════════════════════
...
export function InvoiceDialog({ so, onClose }: Props) {
  const [invoicedBy, setInvoicedBy] = useState('Sadia Karim')
  ...
  useEffect(() => {
    if (so) {
      setInvoicedBy('Sadia Karim')
      setInvoiceNo('') // auto-generate if left blank
      setCartonCount('1')
    }
  }, [so])
  ...
  async function submit() {
    try {
      ...
      if (result.ok) {
        toast.success(`${so!.soNumber} invoiced`, {
          description: `Invoice ${result.invoiceNo} · ${num(Number(cartonCount))} cartons · ${date(new Date())}`,
        })
        onClose()
      } else {
        throw new Error(result.error || 'Failed to invoice')
      }
    } catch (e: any) {
      toast.error('Failed to invoice', { description: e.message })
    }
  }
  ...
  return (
    <WorkflowStepDialog
      ...
      onSubmit={submit}
      submitLabel={patchMutation.isPending ? 'Invoicing…' : 'Generate invoice'}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="text-xs font-medium">Invoice no. (auto if blank)</label>
            <input
          ...
          <div className="sm:col-span-2">
            <label className="text-xs font-medium">Invoiced by</label>
            <input
              className={inputClass + ' mt-1'}
              value={invoicedBy}
